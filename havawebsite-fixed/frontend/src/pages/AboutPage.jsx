...
export const AboutPage = () => {
  ...
  const heroStatKeys = ['years', 'countries', 'facility', 'iso'];
  const capabilityKeys = ['cnc', 'heat', 'assembly', 'testing'];
  const exportRegionKeys = ['africa', 'scandinavia', 'middleEast', 'southAsia'];
  const timelineKeys = ['t1970', 't1990', 't1999', 't2011', 'today'];
  const timelineHighlighted = { t1970: true, t1990: false, t1999: true, t2011: false, today: true };
  const certDetailKeys = ['d1', 'd2', 'd3', 'd4', 'd5', 'd6', 'd7', 'd8', 'd9'];
  const complianceKeys = ['madeInIndia', 'exportStandards', 'atlasCopco', 'qms'];
  const complianceIcons = { madeInIndia: '🇮🇳', exportStandards: '🌍', atlasCopco: '🔧', qms: '📋' };
  const promiseKeys = ['quality', 'support', 'reliability', 'access'];
  const promiseIcons = [ShieldCheck, Layers, Globe, CheckCircle2];
  ...
  return (
    <div className="min-h-screen bg-white overflow-x-hidden">
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center gap-8 lg:gap-12">
            ...
            <div className="flex-1 min-w-0">
              <div className="inline-flex items-center gap-2 bg-white border border-steel-gray rounded-full px-4 py-2 mb-4 shadow-sm">
                ...
                <span className="text-xs font-bold uppercase tracking-[2px] text-charcoal">{t('aboutPage.hero.eyebrow')}</span>
              ...
              <p className="text-base text-gray-600 leading-relaxed max-w-xl">{t('aboutPage.hero.body')}</p>
            </div>
            ...
            <div className="flex-shrink-0 w-full lg:w-80 flex flex-col gap-4">
              ...
              <div className="flex flex-col sm:flex-row lg:flex-col gap-3">
                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="w-full">
                  ...
                  >
                    {t('hero.primaryCTA')} <ArrowRight className="ml-2 w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </Button>
                </motion.div>
                <motion.div whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.97 }} className="w-full">
                  <Button
              ...
              <div className="grid grid-cols-2 gap-2">
                {heroStatKeys.map((key, i) => (
                  ...
                  >
                    <div className="text-base font-black text-charcoal leading-tight">{t(`aboutPage.hero.stats.${key}.value`)}</div>
                    <div className="text-[10px] font-bold text-gray-500 uppercase tracking-wider mt-0.5 leading-tight">{t(`aboutPage.hero.stats.${key}.label`)}</div>
                  </motion.div>
                ))}
              </div>
            </div>
      ...
      <section className="relative py-10 lg:py-14 overflow-hidden bg-white" id="overview">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mt-10">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="lg:col-span-2 space-y-6">
              <div>
                ...
                <p className="text-base text-gray-700 leading-relaxed">{t('aboutPage.companyOverview.intro')}</p>
              </div>
              <div>
                <span className="inline-block text-xs font-bold text-accent-orange uppercase tracking-[2px] mb-3">Brand Story</span>
                <p className="text-base text-gray-700 leading-relaxed">{t('aboutPage.companyOverview.brandStory')}</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="bg-gradient-to-br from-charcoal to-trust-blue rounded-3xl p-7 text-white relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="font-bold text-white mb-2">{t('footer.contact.company')}</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mt-10">
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="bg-white border-2 border-hava-red/20 rounded-3xl p-7 shadow-lg hover:shadow-2xl transition-all relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{t('aboutPage.companyOverview.mission')}</p>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.1 }} className="bg-white border-2 border-trust-blue/20 rounded-3xl p-7 shadow-lg hover:shadow-2xl transition-all relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <p className="text-sm text-gray-700 leading-relaxed">{t('aboutPage.companyOverview.vision')}</p>
              </div>
            </motion.div>
          </div>
          ...
          <div className="mt-10">
            ...
            <p className="text-base text-gray-700 leading-relaxed mb-6 max-w-4xl">{t('aboutPage.companyOverview.manufacturingCapability')}</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {capabilityKeys.map((key, i) => (
                <motion.div key={key} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }} className="bg-gradient-to-br from-slate-50 to-blue-50 border-l-4 border-hava-red rounded-2xl p-5 hover:shadow-xl transition-all">
                  <h5 className="font-bold text-charcoal text-sm mb-2 uppercase tracking-wide">{t(`aboutPage.companyOverview.capabilityCards.${key}.title`)}</h5>
                  <p className="text-xs text-gray-600 leading-relaxed">{t(`aboutPage.companyOverview.capabilityCards.${key}.text`)}</p>
                </motion.div>
              ))}
            </div>
          </div>
          ...
          <div className="mt-10">
            ...
            <p className="text-base text-gray-700 leading-relaxed mb-6 max-w-4xl">{t('aboutPage.companyOverview.exportPresence')}</p>
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {exportRegionKeys.map((key, i) => (
                <motion.div key={key} initial={{ opacity: 0, scale: 0.9 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }} className="bg-white border-2 border-steel-gray rounded-2xl p-5 hover:border-accent-orange/40 hover:shadow-xl transition-all group">
                  ...
                  <h5 className="font-bold text-charcoal text-sm mb-2">{t(`aboutPage.companyOverview.exportRegions.${key}.region`)}</h5>
                  <p className="text-xs text-gray-600 leading-snug">{t(`aboutPage.companyOverview.exportRegions.${key}.text`)}</p>
                </motion.div>
              ))}
            </div>
          </div>
      ...
      <section className="relative py-10 lg:py-14 overflow-hidden bg-gradient-to-br from-slate-50 via-white to-blue-50" id="journey">
        <AnimatedBackground />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeader badge="Sub-Page 2 of 5" badgeColor="hava-red" badgeIcon={Calendar} title="Our" titleGradient="Journey / Timeline" intro={t('aboutPage.journey.intro')} />
          <div className="relative mt-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-5 lg:gap-3">
              {timelineKeys.map((key, i) => {
                const highlighted = timelineHighlighted[key];
                return (
                  <motion.div key={key} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="relative">
                    <div className={`relative z-10 w-12 h-12 mx-auto rounded-full flex items-center justify-center text-white font-black text-xs shadow-lg ${highlighted ? 'bg-gradient-to-br from-hava-red to-accent-orange' : 'bg-charcoal'}`}>
                      {String(i + 1).padStart(2, '0')}
                    </div>
                    <div className={`mt-4 bg-white rounded-2xl p-5 border-t-4 shadow-md hover:shadow-2xl transition-all text-center ${highlighted ? 'border-hava-red' : 'border-steel-gray'}`}>
                      <div className={`text-2xl font-black mb-2 ${highlighted ? 'text-hava-red' : 'text-trust-blue'}`} style={{ fontFamily: 'Space Grotesk, sans-serif' }}>{t(`aboutPage.journey.timeline.${key}.year`)}</div>
                      <h4 className="font-bold text-charcoal text-sm uppercase tracking-wide mb-2">{t(`aboutPage.journey.timeline.${key}.title`)}</h4>
                      <p className="text-xs text-gray-600 leading-relaxed">{t(`aboutPage.journey.timeline.${key}.text`)}</p>
                    </div>
                  </motion.div>
                );
              })}
      ...
      <section className="relative py-10 lg:py-14 overflow-hidden bg-white" id="leadership">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeader badge="Sub-Page 3 of 5" badgeColor="hava-red" badgeIcon={Briefcase} title="Leadership" titleGradient="Team" intro={t('aboutPage.leadership.intro')} />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 max-w-6xl mx-auto mt-10">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-3xl p-7 border-2 border-steel-gray shadow-lg hover:shadow-2xl transition-all relative overflow-hidden">
              ...
              <div className="relative">
                <div className="flex items-start gap-4 mb-5">
                  ...
                  <div>
                    <span className="text-xs font-bold text-hava-red uppercase tracking-wider">{t('aboutPage.leadership.founder.role')}</span>
                    <h3 className="text-2xl font-black text-charcoal mt-1" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>{t('aboutPage.leadership.founder.name')}</h3>
                  </div>
                </div>
                <p className="text-sm text-gray-700 leading-relaxed mb-5">{t('aboutPage.leadership.founder.bio')}</p>
                <div className="flex flex-wrap gap-2">
                  {['t1', 't2', 't3'].map((tk) => (
                    <span key={tk} className="text-xs font-bold uppercase tracking-wider bg-white border border-steel-gray px-3 py-1.5 rounded-full text-charcoal">{t(`aboutPage.leadership.founder.tags.${tk}`)}</span>
                  ))}
                </div>
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="bg-gradient-to-br from-charcoal to-trust-blue rounded-3xl p-7 text-white relative overflow-hidden">
              ...
              <div className="relative">
                ...
                <h3 className="text-xl font-black text-white mb-5" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>{t('aboutPage.leadership.contactTitle')}</h3>
          ...
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }} className="max-w-5xl mx-auto mt-10 bg-gradient-to-br from-hava-red/5 via-white to-accent-orange/5 border-l-4 border-hava-red rounded-r-3xl p-8 lg:p-10 relative">
            ...
            <p className="text-lg lg:text-xl text-charcoal leading-relaxed italic mb-4" style={{ fontFamily: 'Space Grotesk, sans-serif' }}>"{t('aboutPage.leadership.quote')}"</p>
            <cite className="text-sm font-bold text-hava-red uppercase tracking-wider not-italic">{t('aboutPage.leadership.quoteCite')}</cite>
          </motion.div>
        </div>
      </section>
      ...
      <section className="relative py-10 lg:py-14 overflow-hidden bg-gradient-to-br from-slate-50 via-white to-blue-50" id="certifications">
        <AnimatedBackground />
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeader badge="Sub-Page 5 of 5" badgeColor="hava-red" badgeIcon={ShieldCheck} title="Certifications &" titleGradient="Compliance" intro={t('aboutPage.certifications.intro')} />
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-10">
            <motion.div initial={{ opacity: 0, x: -30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="bg-white rounded-3xl border-2 border-steel-gray shadow-xl overflow-hidden">
              ...
              <div className="divide-y divide-steel-gray">
                {certDetailKeys.map((key) => (
                  <div key={key} className="grid grid-cols-1 md:grid-cols-5 gap-2 px-5 py-3 hover:bg-slate-50 transition-colors">
                    <div className="md:col-span-2 text-xs font-bold text-hava-red uppercase tracking-wider">{t(`aboutPage.certifications.details.${key}.label`)}</div>
                    <div className="md:col-span-3 text-sm text-charcoal">{t(`aboutPage.certifications.details.${key}.value`)}</div>
                  </div>
                ))}
              </div>
            </motion.div>
            ...
            <motion.div initial={{ opacity: 0, x: 30 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }} className="space-y-4">
              ...
              {complianceKeys.map((key, i) => (
                <motion.div key={key} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.4, delay: i * 0.1 }} className="bg-white border-l-4 border-hava-red rounded-r-2xl p-5 shadow-md hover:shadow-xl transition-all flex items-start gap-3">
                  <div className="text-2xl flex-shrink-0">{complianceIcons[key]}</div>
                  <div>
                    <h5 className="font-bold text-charcoal text-sm mb-1">{t(`aboutPage.certifications.compliance.${key}.title`)}</h5>
                    <p className="text-xs text-gray-600 leading-relaxed">{t(`aboutPage.certifications.compliance.${key}.text`)}</p>
                  </div>
                </motion.div>
              ))}
            </motion.div>
      ...
      <section className="relative py-10 lg:py-14 overflow-hidden bg-white">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <SectionHeader badge={t('aboutPage.promise.label')} badgeColor="hava-red" badgeIcon={Sparkles} title="Dependable Rock Drilling Solutions." titleGradient="Every Time. Every Year." intro={t('aboutPage.promise.body')} />
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5 mt-10">
            {promiseKeys.map((key, i) => {
              const Icon = promiseIcons[i % promiseIcons.length];
              return (
                <motion.div key={key} initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.1 }} className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-3xl p-6 border-2 border-steel-gray hover:border-hava-red/40 shadow-md hover:shadow-2xl transition-all card-hover group">
                  ...
                  <h3 className="font-bold text-charcoal text-base mb-2 group-hover:text-hava-red transition-colors">{t(`aboutPage.promise.cards.${key}.title`)}</h3>
                  <p className="text-sm text-gray-600 leading-relaxed">{t(`aboutPage.promise.cards.${key}.text`)}</p>
                </motion.div>
              );
            })}
          </div>
      ...
      <section className="relative py-10 lg:py-14 overflow-hidden bg-charcoal">
        ...
        <div className="relative max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-base text-white/70 mb-8 max-w-2xl mx-auto">{t('aboutPage.finalCta.body')}</p>
